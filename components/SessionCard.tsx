...
const SessionCard: React.FC<SessionCardProps> = ({ session, sessionIndex, onStartSession, dayId }) => {
  const { getTaskStatus } = useAppContext();

  let completedTasksInSession = 0;
  session.tasks.forEach(task => {
    const status = getTaskStatus(task.id);
    if (status?.isCorrect || (status?.answer === 'completed' && task.answerInputType === 'parent_check')) {
  ...
  return (
    <div className={`p-4 sm:p-6 rounded-xl ${ // Removed shadow-lg and border
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {session.tasks.map(task => {
           const status = getTaskStatus(task.id);
           const isTaskCorrect = status?.isCorrect || (status?.answer === 'completed' && task.answerInputType === 'parent_check');
